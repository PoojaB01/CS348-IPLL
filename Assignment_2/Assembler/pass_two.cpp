...
int pass_two(FILE *inputFile, FILE *outputFile, FILE *listingFile)
{
    ...
    while (strcmp(OPCODE, "END") != 0 || true)
    {
        ...
        if (line[1] != '.') // check line not a comment
        {
            if (strcmp(OPCODE, "CSECT") == 0)
            {
                ...
                if (first_sect)
                {
                    fprintf(outputFile, "E%06X", STARTADDR);
                    first_sect = false;
                }
                else
                ...
                else if (info->format == 3 && !extended)
                {
                    ...
                    if (words > 2)
                    {
                        ...
                        if (!isdigit(OPERAND[0]))
                        {
                            ...
                            operand_value = handle_operand(OPERAND, modification_records, base, 5, (int)strtol(addr, NULL, 16))
                                            - (int)strtol(addr, NULL, 16) - 3;
                            if (operand_value < 0)
                            {
                                operand_value += 1 << 12;
                            }
            ...
            {
                sprintf(objcode, "%06X", handle_operand(OPERAND, modification_records, base, 6, (int)strtol(addr, NULL, 16)));
            }
            ...
            {
                sprintf(addr, "%04X", handle_operand(OPERAND, modification_records, base, 4, (int)strtol(addr, NULL, 16)));
            }
    ...
    if (first_sect)
    {
        fprintf(outputFile, "%06X", STARTADDR);
        first_sect = false;
    }
    fprintf(outputFile, "\n\n\n");
