...
int pass_one(FILE *inputFile, FILE *outputFile)
{
    ...
    while (strcmp(OPCODE, "END") != 0)
    {
        if (line[0] != '.') // check if not a comment
        {
            ...
            {
                // print all literals encountered
                LOCCTR = print_literals(LOCCTR, outputFile, base);

                // set length of the previous control section
            ...
            bool extended = false;
            // check if instruction in extended format
            if (OPCODE[0] == '+')
            {
                extended = true;
                OPCODE++;
        ...
        {
            // write comment in intermediate file
            fprintf(outputFile, "    \t%s", line);
        }
