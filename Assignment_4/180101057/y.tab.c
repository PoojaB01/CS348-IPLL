...
{
       0,    54,    54,    57,    60,    61,    62,    65,    76,    80,
      85,    89,    96,    97,    98,   101,   102,   103,   104,   107,
     123,   126,   139,   154,   157,   170,   185,   194,   197,   200,
     205,   215,   225,   226,   229,   245,   246
};
#endif
...
{
  "$end", "error", "$undefined", "PROGRAM", "VAR", "_BEGIN", "END",
  "END_DOT", "INTEGER", "REAL", "FOR", "READ", "WRITE", "TO", "DO",
  "SEMICOLON", "COLON", "COMMA", "EQUAL", "ADD", "SUB", "MUL", "DIV",
  "OPENBRACKET", "CLOSEBRACKET", "CHARACTER", "ID", "INT", "REAL_NUM",
  "$accept", "prog", "progname", "declist", "dec", "type", "idlist",
  "stmtlist", "stmt", "assign", "exp", "term", "factor", "read", "write",
  "for", "indexexp", "body", YY_NULLPTR
};
#endif
...
#line 65 "180101057.y"
                        {
                            ...
                            if(declare_type((yyvsp[-2].str), (yyvsp[0].type)) < 0)
                            {
...
#line 1451 "y.tab.c"
  ...
  case 19:
#line 107 "180101057.y"
                            {   
                                // if LHS not declared
                                if(getType((yyvsp[-2].str)) == not_defined)
                                {
                                    fprintf (stderr, "Line %d : semantic error, %s\n", yylineno, "symbol not decalred.");
                                    ERROR_FLAG = 1;
                                }
                                // if mismatch in LHS and RHS type
                                else if(getType((yyvsp[-2].str)) != (yyvsp[0].type) && (yyvsp[0].type) != not_defined)
                                {
...
#line 1596 "y.tab.c"
  ...
  case 30:
#line 205 "180101057.y"
                                                {
                                                    // check if all ids are valid
                                                    if(verify_ids((yyvsp[-1].str)) < 0)
                                                    {
                                                        fprintf (stderr, "Line %d : semantic error, %s\n", yylineno, "symbol not declared.");
                                                        ERROR_FLAG = 1;
                                                    }
...
#line 1622 "y.tab.c"
  ...
  case 34:
#line 229 "180101057.y"
                                    {
                                        // check if id not declared
                                        if(getType((yyvsp[-4].str)) == not_defined)
                                        {
                                            fprintf (stderr, "Line %d : semantic error, %s\n", yylineno, "symbol not declared in for loop index expression.");
                                            ERROR_FLAG = 1;
                                        }
                                        // check for type mismatch
                                        else if(((yyvsp[-2].type) != (yyvsp[0].type) || getType((yyvsp[-4].str)) != (yyvsp[-2].type) || getType((yyvsp[-4].str)) != (yyvsp[0].type)) && (yyvsp[-2].type) != not_defined && (yyvsp[0].type) != not_defined)
                                        {
